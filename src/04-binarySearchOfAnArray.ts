...
function binarySearch(arr: number[], targetValue: number): string {
  ...
  //  What would happen if the number you are looking for is not in the arr?
    let guess;
    while (min <= max) {
        // Find the mid index and round using Math.floor
        let guess = Math.floor((min + max) / 2);

        //If element is present at mid, return guess;
  ...
  // return index of location where the target value was found
}
